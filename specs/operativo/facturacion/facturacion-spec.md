...
El sistema debe generar automáticamente, tras un pago exitoso, un registro en `comisiones` con `estado = pendiente_cobro` y el monto pactado con la aerolínea (`aerolineas.comision_pactada_pct` sobre el monto correspondiente), reflejando el modelo de ingreso diferido descrito en `consideraciones.md` (sección 5).
...
- Configuración de reglas de tarifas de servicio y comisiones, y de políticas de reembolso por nivel de tarifa (CU-T09, CU-T10) — nivel Táctico previsto; este módulo consume los valores ya persistidos.
- Configuración de credenciales de Stripe (CU-T08) y catálogo de aerolíneas/comisión pactada (CU-T12) — nivel Táctico previsto.
- Dashboard financiero con KPIs de ingresos y comisiones (alcance futuro, sin CU redactados aún).
