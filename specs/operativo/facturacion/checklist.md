...
- [x] CHK031 RF-FAC-012 (CU-O86) — captura de pago diferido de hotel. **Completo 2026-07-22** junto con RF-HOT-009 (CU-O60, `hoteles-spec.md`) — `pago_service._autorizar_pago`/`capturar_pago_diferido`, `payment_gateway.autorizar`/`capturar` (Stripe `capture_method=manual`). 8 tests reales contra Stripe test mode en `app/facturacion/tests/test_pago_diferido.py`, más 3 en `app/carrito/tests/test_modalidad_pago.py`. Backoffice: `/backoffice/pagos-diferidos` (listar + capturar), disparo manual porque HotelLens no expone una señal real de confirmación del hotel (mismo criterio que CU-O48 "Forzar estado"). Bug preexistente corregido de paso: `procesar_pago`/`obtener_vuelo` con `vuelo_id=""` pegaba contra el endpoint de listar en vez de 404ear, rompiendo `registrar_comision` para cualquier reserva sin vuelo (hotel/auto/actividad/crucero) — nunca antes ejercitado end-to-end.
