...
async def _limpiar_reserva(pb, reserva_id: str) -> None:
    ...
    extras = await pb.list_records("reserva_extras", {"filter": f'reserva_id="{reserva_id}"'})
    for extra in extras["items"]:
        await pb.delete_record("reserva_extras", extra["id"])
    await pb.delete_record("reservas", reserva_id)


async def test_iniciar_paquete_crea_reserva_con_un_solo_item(pb, pasajero_factory, vuelo_factory, tarifa_factory):
    ...
    assert items["totalItems"] == 1
    assert items["items"][0]["tipo_producto"] == "vuelo"
...
async def test_confirmar_paquete_copia_descuento_y_aplica_precio_final(
    ...
    items = await pb.list_records("reserva_items", {"filter": f'reserva_id="{reserva["id"]}"'})
    assert {i["precio_final"] for i in items["items"]} == {200.0, 300.0}
...
async def test_confirmar_paquete_sin_hotel_rechaza(pb, pasajero_factory, vuelo_factory, tarifa_factory):
    _usuario, pasajero = await pasajero_factory()
    vuelo = await vuelo_factory()
    tarifa = await tarifa_factory(vuelo["id"], precio_final=200.0)
...
async def test_cambiar_componente_no_afecta_los_demas(pb, pasajero_factory, vuelo_factory, tarifa_factory):
    ...
    items = await pb.list_records("reserva_items", {"filter": f'reserva_id="{reserva["id"]}"'})
    assert items["totalItems"] == 2  # sigue siendo vuelo + hotel, no se duplicó ni se perdió el vuelo
    hotel_actualizado = next(i for i in items["items"] if i["tipo_producto"] == "hotel")
...
async def test_agregar_traslado_aeropuerto(pb, pasajero_factory, vuelo_factory, tarifa_factory):
    _usuario, pasajero = await pasajero_factory()
    vuelo = await vuelo_factory()
    tarifa = await tarifa_factory(vuelo["id"], precio_final=200.0)
...
async def test_agregar_componente_de_reserva_ajena_rechaza(pb, pasajero_factory, vuelo_factory, tarifa_factory):
