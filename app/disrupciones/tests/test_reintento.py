...
async def _crear_notificacion_fallida(pb, pasajero_id: str, reserva_id: str, intentos_envio: int) -> dict:
...
async def test_notificacion_fallida_se_reintenta_y_puede_recuperarse(
    ...
    notificacion = await _crear_notificacion_fallida(pb, pasajero["id"], reserva["id"], intentos_envio=1)
    ...
    actualizada = await pb.get_record("notificaciones", notificacion["id"])
...
async def test_reintentos_agotados_quedan_como_fallo_definitivo_visible(
    ...
    notificacion = await _crear_notificacion_fallida(pb, pasajero["id"], reserva["id"], intentos_envio=2)
...
async def test_no_hay_reintento_indefinido_tras_agotar_el_limite(
    ...
    pasajero = vuelo_con_reserva_confirmada["pasajero"]
    reserva = vuelo_con_reserva_confirmada["reserva"]
    notificacion = await _crear_notificacion_fallida(pb, pasajero["id"], reserva["id"], intentos_envio=3)
...
async def test_canal_no_disponible_no_lanza_excepcion_ni_afecta_el_resto(
    pb, vuelo_con_reserva_confirmada
):
