...
async def test_agregar_item_crea_carrito_activo_si_no_existe(pb, pasajero_factory, vuelo_factory, tarifa_factory):
    ...
    carrito = await pb.get_first("carritos", f'pasajero_id="{pasajero["id"]}" && estado="activo"')
...
async def test_agregar_dos_items_reutiliza_el_mismo_carrito(pb, pasajero_factory, vuelo_factory, tarifa_factory):
    ...
    carritos = await pb.list_records("carritos", {"filter": f'pasajero_id="{pasajero["id"]}" && estado="activo"'})
    assert carritos["totalItems"] == 1  # nunca se crea un segundo carrito en paralelo

    await pb.delete_record("carrito_items", item_a["id"])
    await pb.delete_record("carrito_items", item_b["id"])
    await pb.delete_record("carritos", item_a["carrito_id"])


async def test_ver_carrito_suma_precios_snapshot(pb, pasajero_factory, vuelo_factory, tarifa_factory):
    _usuario, pasajero = await pasajero_factory()
    vuelo = await vuelo_factory()
    tarifa_a = await tarifa_factory(vuelo["id"], precio_final=100.0)
    tarifa_b = await tarifa_factory(vuelo["id"], precio_final=150.0)
    ...
    for item in resultado["items"]:
        await pb.delete_record("carrito_items", item["id"])
    await pb.delete_record("carritos", resultado["carrito"]["id"])
...
async def test_eliminar_item_no_afecta_los_demas(pb, pasajero_factory, vuelo_factory, tarifa_factory):
    _usuario, pasajero = await pasajero_factory()
    vuelo = await vuelo_factory()
    tarifa_a = await tarifa_factory(vuelo["id"], precio_final=100.0)
    tarifa_b = await tarifa_factory(vuelo["id"], precio_final=150.0)
...
async def test_eliminar_item_de_otro_pasajero_rechaza(pb, pasajero_factory, vuelo_factory, tarifa_factory):
    ...
    carrito = await pb.get_first("carritos", f'pasajero_id="{pasajero_a["id"]}"')
    await pb.delete_record("carrito_items", item["id"])
    await pb.delete_record("carritos", carrito["id"])
