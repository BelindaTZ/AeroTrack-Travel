"""Siembra idempotente del RBAC del módulo Integraciones (CU-T37/T38):
`modulos`, `permisos` (ver/editar/ejecutar), `roles_permisos` (solo
Administrador — el catálogo dice explícitamente "Actor: Administrador",
igual que Seguridad/Configuración no aparecen bajo Agente/Pasajero en
`seed_seguridad.py`) y `modulo_tablas`. Mismo patrón de upsert por clave
natural que `seed_seguridad.py` — sin tocar ese script para no re-ejecutar
el seed de los 6 módulos originales.

Requiere que `seed_seguridad.py` ya haya corrido (rol "Administrador"
sembrado).

Ejecutar: python scripts/seed_integraciones_rbac.py
"""
...
MODULO_CLAVE = "integraciones"
ACCIONES = ["ver", "editar", "ejecutar"]
TABLAS = ["fuentes_datos_externas", "sincronizaciones_log"]
...
def main() -> None:
    ...
    if modulo is None:
        modulo = create(
            ...
            {
                "clave": MODULO_CLAVE,
                "nombre_display": "Integraciones",
                "descripcion": "Configuración y bitácora de fuentes de datos externas (CU-T37/T38)",
                "orden": 80,
            },
        )
