"""Siembra idempotente del RBAC de Autos (CU-T11, reporte de reservas por
proveedor/categoría — Actor: Administrador). Mismo patrón de upsert que
`seed_ofertas_rbac.py`; Autos no tenía ningún RBAC sembrado todavía (el
módulo nunca tuvo pantalla de backoffice hasta este ítem).

Requiere que `seed_seguridad.py` ya haya corrido (rol "Administrador"
sembrado).

Ejecutar: python scripts/seed_autos_rbac.py
"""
...
def main() -> None:
    ...
    if modulo is None:
        modulo = create(
            headers, "modulos",
            {
                "clave": MODULO_CLAVE, "nombre_display": "Autos",
                "descripcion": "Catálogo de renta de autos y reporte de reservas por proveedor/categoría (CU-T11)",
                "orden": 110,
            },
        )
