"""Siembra idempotente del RBAC de Asistente IA (CU-T33/T34): `modulos`,
`permisos` (ver/editar), `roles_permisos` solo para Administrador — el
catálogo dice explícitamente "Actor: Administrador" para ambas
funcionalidades tácticas de este módulo. Mismo patrón que
`seed_ofertas_rbac.py`.

Requiere que `seed_seguridad.py` ya haya corrido (rol "Administrador"
sembrado).

Ejecutar: python scripts/seed_asistente_ia_rbac.py
"""
...
MODULO_CLAVE = "asistente_ia"
ACCIONES = ["ver", "editar"]
TABLAS = ["conversaciones_ia", "mensajes_ia"]
...
def main() -> None:
    ...
    if modulo is None:
        modulo = create(
            headers, "modulos",
            {
                "clave": MODULO_CLAVE, "nombre_display": "Asistente IA",
                "descripcion": "Configuración del asistente y reporte de consultas (CU-T33/T34)",
                "orden": 110,
            },
        )
