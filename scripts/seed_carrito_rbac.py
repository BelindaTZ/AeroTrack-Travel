"""Siembra idempotente del RBAC de Carrito Táctico (CU-T26 configurar
abandono, CU-T27 reporte de recuperación — Actor: Administrador). Mismo
patrón de upsert que `seed_autos_rbac.py`; el nivel Operativo de Carrito
(CU-O93-O96) es autoservicio del pasajero y nunca tuvo pantalla de
backoffice, así que tampoco tenía RBAC sembrado todavía.

Requiere que `seed_seguridad.py` ya haya corrido (rol "Administrador"
sembrado).

Ejecutar: python scripts/seed_carrito_rbac.py
"""
...
def main() -> None:
    ...
    if modulo is None:
        modulo = create(
            headers, "modulos",
            {
                "clave": MODULO_CLAVE, "nombre_display": "Carrito",
                "descripcion": "Configuración de recuperación de carrito abandonado y reporte de tasa de recuperación (CU-T26/T27)",
                "orden": 115,
            },
        )
